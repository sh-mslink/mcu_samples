...
int main(void)
{
    ...
    //check HDC2080 ID
    ret = readHDC2080ID(i2cRxBuf, 2);
    if(ret == 0)
    {
        printf("ERROR: HDC2080 not found.\n");
        return 0;
    }
    else
        printf("HDC2080 ID: 0x%02X%02X\n", i2cRxBuf[0], i2cRxBuf[1]);
    
    //BLE init
    ret = ble_init();
    if(ret == 0)
    {
        printf("ERROR: BLE init fail.\n");
        return 0;
    }
    else
