...
uint8_t SpiFlash_Init(void)
{
  ...
  SPI_InitStructure.SPI_TransMode = SPI_TRANSMODE_FULLDUPLEX;  //全双工模式
  SPI_InitStructure.SPI_CPHA = SPI_CPHA_2EDGE;   //第二个时钟边沿采样数据
  SPI_InitStructure.SPI_CPOL = SPI_CPOL_HIGH;  //总线空闲状态时时钟线为高
  SPI_InitStructure.SPI_CPOLY = 0;      //指定用于CRC校验的多项式，这里不不开启
  SPI_InitStructure.SPI_FirstBit = SPI_FIRSTBIT_MSB;  //数据收发高位在前
  SPI_InitStructure.SPI_FrameSize = SPI_FRAMESIZE_8BIT;  //数据以字节(8bit)为单位
  SPI_InitStructure.SPI_MCLKP = SPI_MCLKP_32;  //SPI 时钟32分频
  SPI_InitStructure.SPI_NSSSEL = SPI_NSSSEL_SOFT; //软件控制CS管脚
  SPI_InitStructure.SPI_Mode = SPI_MODE_MASTER; //单片机时主机模式
...
{ 
  ...
  wSectorRemain = SPIF_SECTOR_SIZE - wSectorOffset; // the remain in a sector 
 	if(dwNumByteToWrite <= wSectorRemain) //确保要写入的数据大小是sector的整数倍
  {
    wSectorRemain = dwNumByteToWrite; // smaller than a sector size  //如果不是整数倍那么就凑满
  }
  while(1) 
  {	
		//为了可靠操作，在任何写入操作前先要对该区域做个判断，看看是不是真的被擦出过
		//Flash被擦除后，所有的字节都是0xFF
    ...
    if(dwNumByteToWrite == wSectorRemain)
    {
      break; // write end
		...
		//如果要写入的数据不是sector大小的整数倍，那么需要凑齐再写
    else // go on writing
    {
      dwSectorPos++;
      wSectorOffset = 0;	 
...
{
  FLASH_CS_LOW();  //拉低片选
  SPI_WriteByte(SPIF_ReadData); // 发送读数据指令
  SPI_WriteByte((uint8_t)((dwReadAddr) >> 16)); //发送24bit地址中的最高字节
  SPI_WriteByte((uint8_t)((dwReadAddr) >> 8)); //发送24bit地址中的中间字节
  SPI_WriteByte((uint8_t)dwReadAddr); //发送24bit地址中的低字节
  SPI_ReadBytes(pbBuffer,dwNumByteToRead); //读取数据
  FLASH_CS_HIGH(); //拉高片选
}
...
void SpiFlash_Erase_Sector(uint32_t dwDstAddr)
{
  ...
  SpiFlash_Write_Enable();   //发送指令使能擦除操作
  SpiFlash_Wait_Busy();  //读取Flash中的状态寄存器，直到Flash空闲下来
  FLASH_CS_LOW(); //拉低CS
  SPI_WriteByte(SPIF_SectorErase); //发送擦除指令
  SPI_WriteByte((uint8_t)((dwDstAddr) >> 16));  //发送擦除操作的地址
  SPI_WriteByte((uint8_t)((dwDstAddr) >> 8));
  SPI_WriteByte((uint8_t)dwDstAddr);
  FLASH_CS_HIGH(); //拉高CS
  SpiFlash_Wait_Busy(); //Flash的擦除和写操作耗时都比较长，一般要到几个mS，相对于单片机而言非常长久，因此要等待Flash操作完成
}
...
uint16_t SpiFlash_ReadID(void)
{
  ...
  FLASH_CS_LOW(); //拉低片选
  SPI_WriteByte(SPIF_ManufactDeviceID); //发送读取ID指令
  SPI_WriteByte(0x00);  //根据Flash数据手册说明，发送dummy数据
  SPI_WriteByte(0x00);  //根据Flash数据手册说明，发送dummy数据
  SPI_WriteByte(0x00);  //发送读取ID数据的模式
  wReceiveData|=SPI_ReadByte() << 8; //读取高字节数据
  wReceiveData|=SPI_ReadByte();  //读取低字节数据
  FLASH_CS_HIGH(); //拉高片选
	return wReceiveData;
}  
