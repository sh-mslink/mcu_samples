...
extern u32 gTimerMS;	
/**
  * @brief  reset SysTick value for time testing in microsecond and enable interrupt
  * @param  None
  * @retval None
  */
void enableSysTickInt(void)
{
  ...
  //SysTick->LOAD = (u32)(fac_us-12);
  //SysTick->VAL = 0x00;
  //SysTick->CTRL |= (SysTick_CTRL_ENABLE_Msk | SysTick_CTRL_TICKINT_Msk);  //enable SysTick and Interrupt
}
	
/**
  * @brief  get current time based on SysTick interrupt
  * @param  mS_or_uS: 1 means return value by uS, 0 means return value by mS
  * @retval return time value
  */
u32 getSysTick(u8 mS_or_uS)
{
  u32 temp;
...
/**
  * @brief  stop SysTick and disable interrupt, at same time return current value
  * @param  mS_or_uS: 1 means return value by uS, 0 means return value by mS
  * @retval return time value
  */
u32 stopSysTick(u8 mS_or_uS)
{ 
	u32 temp;
