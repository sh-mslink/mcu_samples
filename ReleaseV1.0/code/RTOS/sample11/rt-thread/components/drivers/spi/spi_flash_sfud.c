...
//Sean add follow code for SPI flash device attach during device initialization
static int spi_device_init(void)
{
  if (rt_hw_spi_device_attach("spi2", "exFlash", GPIOB, GPIO_Pins_12)!= RT_EOK)
	{
		rt_kprintf("spi2 device attach error\r\n");
	}
	else
	{
		rt_kprintf("spi2 divece attach success\r\n");
	}
    return RT_EOK;
}
